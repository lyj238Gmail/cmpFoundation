...
"VF \<equiv>{(Ident ''AuxData''),(Field (Para (Ident ''Cache'') 0) ''Data''),(Field (Para (Ident ''Cache'') 0) ''State''),
(Field (Para (Ident ''Cache'') 1) ''Data''),(Field (Para (Ident ''Cache'') 1) ''State''),(Field (Para (Ident ''Chan1'') 0) ''Cmd''),
(Field (Para (Ident ''Chan1'') 1) ''Cmd''),(Field (Para (Ident ''Chan2'') 0) ''Cmd''),(Field (Para (Ident ''Chan2'') 0) ''Data''),
(Field (Para (Ident ''Chan2'') 1) ''Cmd''),(Field (Para (Ident ''Chan2'') 1) ''Data''),(Field (Para (Ident ''Chan3'') 0) ''Cmd''),
(Field (Para (Ident ''Chan3'') 0) ''Data''),(Field (Para (Ident ''Chan3'') 1) ''Cmd''),(Field (Para (Ident ''Chan3'') 1) ''Data''),
(Ident ''CurCmd''),(Ident ''ExGntd''),(Para (Ident ''InvSet'') 0),(Para (Ident ''InvSet'') 1),(Ident ''MemData''),
(Para (Ident ''ShrSet'') 0),(Para (Ident ''ShrSet'') 1)}"
...
from a4  have tmp:"formEval (inv_91)  s"   
 ...
 next
   from a1 show "\<forall>v. v \<in> varOfForm (pre (?r')) \<longrightarrow>
     formEval (pre (?r)) s \<longrightarrow> v \<in> VF  \<or> v ∈ VF' \<and> scalar2Nat (s v) ≤ NC" 
		by(cut_tac a1 a3, auto)
...
proof(rule ruleSimCond3)
 ...
 v \<in> VF  \<or> v ∈ VF' \<and> scalar2Nat (s v) ≤ NC" 
		apply(cut_tac a1 a3, auto) done
...
proof(unfold trans_sim_on3_def,(rule allI)+,(rule impI)+,rule disjI2)
  ...
  proof(cut_tac a1,unfold state_sim_on3_def,rule conjI)
    show "∀v. v ∈ VF ⟶ trans (act ?r) s v = s2 v"
    proof(rule allI,rule impI)
    fix f v
    assume b1:" v \<in>  VF"  
  ...
  qed
next
  show " ∀v. v ∈ VF' ⟶ scalar2Nat (trans (act ?r) s v) ≤ NC ⟶ trans (act ?r) s v = s2 v" 
    by(cut_tac b0, auto)
...
lemma lemmaOnn_RecvGntE1LeNc_:
  assumes a1:"i\<le>NC"  and a3:"NC\<le>N" 
  shows "trans_sim_on3 (n_RecvGntE1  i) (n_RecvGntE1  i) VF  VF' NC s" (is "trans_sim_on3 ?r ?r' VF ?F NC s")
proof(rule ruleSimCond3)
     from a3 show " formEval (pre ?r) s \<longrightarrow>formEval (pre ?r') s" (is "?A \<longrightarrow>?B")  
       apply (auto simp del:evalEqn)done
   next 
 ...
 next
   show "\<forall>  v. v \<in>  varOfSent (act ?r) \<longrightarrow>  v \<in> VF \<longrightarrow> v \<in>  varOfSent (act ?r')" 
		by(cut_tac a1 a3, auto)
 next
   from a1 show "\<forall>v. v \<in> varOfForm (pre (?r')) \<longrightarrow> formEval (pre ?r) s \<longrightarrow> v \<in> VF
  \<or> v ∈ VF' \<and> scalar2Nat (s v) ≤ NC" 
		by(cut_tac a1 a3, auto)
  
next 
  show "\<forall>v. v \<in> varOfSent (act ?r)\<longrightarrow>
        v \<in> VF'\<longrightarrow>  formEval (pre ?r) s \<longrightarrow>scalar2Nat (expEval (substExpByStatement (IVar v) (act ?r)) s) \<le> NC
       \<longrightarrow> v \<in> varOfSent (act ?r')"
    	by(cut_tac a1 a3, auto)
next
  show "\<forall> v va. v \<in> varOfSent (act ?r') \<longrightarrow> 
...
proof(rule ruleSimCond2)
  ...
  show "expEval (substExpByStatement (IVar v)  (act ?r')) s = expEval (substExpByStatement (IVar v)  (act ?r)) s"  
       apply (cut_tac  a1 a3 b1 b2 b3,auto) done
   qed
 next
   show "\<forall>  v. v \<in>  varOfSent (act ?r) \<longrightarrow>  v \<in> VF \<longrightarrow> v \<in>  varOfSent (act ?r')" 
		by(cut_tac a1 a3, auto)
 ...
 next 
   show "\<forall> v va. v \<in> varOfSent (act ?r') \<longrightarrow>va\<in>varOfExp ( substExpByStatement (IVar v)  (act ?r'))
\<longrightarrow> va \<in> VF "
     apply(cut_tac a1 a3, auto) done
 next
   from a1 show "\<forall>v. v \<in> varOfForm (pre (?r')) \<longrightarrow> v \<in> VF" 
		by(cut_tac a1 a3, auto)
 qed
...
from a4  have tmp:"formEval (inv_2 0 1)  s"   
      ...
      from a1 a3 b0 c0 c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 c26 c27 c28 c29 c30 c31 c32 c33 c34 c35 c36 c37 c38 c39 c40 c41 c42 c43 c44 c45 c46 c47 show "formEval (pre ?r') s" 
       by (auto simp del:evalEqn)
     qed
   next 
	show "(\<forall>  v. v \<in>  varOfSent (act  ?r') \<longrightarrow>  v \<in> VF \<longrightarrow> formEval (pre ?r) s \<longrightarrow> 
...
from a4  have tmp:"formEval (inv_134 1)  s"   
      ...
      from a1 a3 b0 c0 c1 c2 c3 c4 c5 show "formEval (pre ?r') s" 
       by (auto simp del:evalEqn)
     qed
   next 
	show "(\<forall>  v. v \<in>  varOfSent (act  ?r') \<longrightarrow>  v \<in> VF \<longrightarrow> formEval (pre ?r) s \<longrightarrow> 
...
lemma lemmaOnn_RecvReqE11LeNc_:
  assumes a1:"i\<le>NC"  and a3:"NC\<le>N" 
  shows "trans_sim_on2 (n_RecvReqE11 N i) (n_RecvReqE11 NC i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(rule ruleSimCond2)
     from a3 show " formEval (pre ?r) s \<longrightarrow>formEval (pre ?r') s" (is "?A \<longrightarrow>?B")  
       apply (auto simp del:evalEqn)done
   next 
...
lemma lemmaOnn_RecvReqS12LeNc_:
  assumes a1:"i\<le>NC"  and a3:"NC\<le>N" 
  shows "trans_sim_on2 (n_RecvReqS12 N i) (n_RecvReqS12 NC i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(rule ruleSimCond2)
     from a3 show " formEval (pre ?r) s \<longrightarrow>formEval (pre ?r') s" (is "?A \<longrightarrow>?B")  
       apply (auto simp del:evalEqn)done
   next 
...
from a4  have tmp:"formEval (inv_191 0)  s"   
        ...
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c12:"formEval  (conclude (inv_191 i d)) s" by auto
