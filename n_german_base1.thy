...
theory n_german_base1 imports paraTheory
begin
...
let g = (andForm (andForm (andForm (andForm (andForm (eqn (IVar (Ident ''CurCmd'')) (Const ReqE)) (eqn (IVar (Ident ''ExGntd'')) (Const false)))
 (forallForm (down NC) (\<lambda>j. (eqn (IVar (Para (Ident ''ShrSet'') j)) (Const false)))))
 (eqn (IVar (Ident ''MemData'')) (IVar (Ident ''AuxData'')))) 
(forallForm (down NC) (\<lambda>i. (neg (eqn (IVar (Field (Para (Ident ''Chan2'') i) ''Cmd'')) (Const GntE))))))
 (forallForm (down NC) (\<lambda>j. (neg (eqn (IVar (Field (Para (Ident ''Cache'') j) ''State'')) (Const E)))))) in
...
definition inv_154::"nat \<Rightarrow> formula" where [simp]:
"inv_154 j \<equiv>
(implyForm (eqn (IVar (Field (Para (Ident ''Cache'') j) ''State'')) (Const I)) (neg (eqn (IVar (Field (Para (Ident ''Chan2'') j) ''Cmd'')) (Const Inv))))"
...
(\<exists> j. j\<le>N\<and>f=inv_154  j) \<or>
...
axiomatization  where axiomOnf2 [simp,intro]:
   "s ∈ reachableSet (set (allInitSpecs N )) (rules N) ⟹  1 < N ⟹ i < 2 ⟹ j<2 ⟹ i\<noteq>j\<Longrightarrow> formEval (f 0 1) s ⟹ formEval (f i j) s"
...
lemma lemmaOnn_RecvGntE1: 
  ...
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_RecvGntE1  i"  and
  a6:"NC<N"
...
lemma lemmaOnn_RecvGntS2: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_RecvGntS2  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendGntE3Gt_i:
  ...
  from a4  have tmp:"formEval (inv_239 0)  s"   
    ...
    from a4  have tmp1:"formEval (inv_239 1)  s"   
      by (force simp del:inv_239_def)

     
        have tmp2:"formEval (inv_239 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 tmp tmp1 have c0:"formEval  (conclude (inv_239 0)) s & formEval  (conclude (inv_239 1)) s" by auto 

        from a4  have tmp:"formEval (inv_232 0)  s"   
          by (force simp del:inv_232_def)                                      

        from a4  have tmp1:"formEval (inv_232 1)  s"   
          by (force simp del:inv_232_def)

        have tmp2:"formEval (inv_232 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
       
        with b0 a1 tmp tmp1 have c1:"formEval  (conclude (inv_232 0)) s & formEval  (conclude (inv_232 1)) s" by auto
        from a4 have tmp:"formEval inv_91 s"  by (force simp del:inv_91_def) 
         with b0 a1 have c2:"formEval  (conclude (inv_91)) s" by auto
      from b0 c0 c1 c2 a1 a3 show "formEval (pre ?r') s" 
        apply( auto)
        apply(auto intro:forallLemma1) done
     qed
   next 
...
proof(rule ruleSimCond2)
 ...
 next 
   show "∀ v va. v ∈ varOfSent (act ?r') ⟶va∈varOfExp ( substExpByStatement (IVar v)  (act ?r'))⟶ va ∈ VF "
     by auto  
 next
   from a1 show "∀v. v ∈ varOfForm (pre (?r')) ⟶ v ∈ VF" by auto
 qed
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendGntE3: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendGntE3  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
  from a5 obtain i where d0:"i\<le>N\<and>r=n_SendGntE3  i"  by blast
  have "i>NC|i\<le> NC" by auto
  moreover{
       assume a1:"i>NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
...
moreover{
       assume a1:"i\<le> NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
        proof(rule_tac x="(n_SendGntE3 N i)" in exI,rule conjI)
          show  "?P1 (n_SendGntE3 N i) " 
           by(cut_tac a1, auto) 
          next
          show  "?P2 (n_SendGntE3 N i) "
...
lemma lemmaOnn_SendGntS4LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendGntS4  i) (n_SendGntS4  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendGntS4: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendGntS4  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_RecvInvAck5Gt_i:
  ...
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC<N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
  shows "trans_sim_on2 (n_RecvInvAck5  i)  (n_RecvInvAck5_i_3 ) VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF (set ?F) s")
  proof(rule ruleSimCond2)
    show " formEval (pre ?r) s ⟶formEval (pre ?r') s" (is "?A ⟶?B")
    proof(rule impI)+
      assume b0:"?A"
  from a4  have tmp:"formEval (inv_233 0)  s"   
            by (force simp del:inv_233_def) 
        have tmp1:"formEval (inv_233 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c0:"formEval  (conclude (inv_233 i)) s" by auto
from a4  have tmp:"formEval (inv_223 0)  s"   
            by (force simp del:inv_223_def) 
        have tmp1:"formEval (inv_223 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c1:"formEval  (conclude (inv_223 i)) s" by auto
from a4  have tmp:"formEval (inv_219 0)  s"   
            by (force simp del:inv_219_def) 
        have tmp1:"formEval (inv_219 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c2:"formEval  (conclude (inv_219 i)) s" by auto
from a4  have tmp:"formEval (inv_218 0 1)  s"   
            by (force simp del:inv_218_def) 
        have tmp1:"formEval (inv_218 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c3:"formEval  (conclude (inv_218 i 0)) s" by auto
from a4  have tmp:"formEval (inv_218 0 1)  s"   
            by (force simp del:inv_218_def) 
        have tmp1:"formEval (inv_218 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c4:"formEval  (conclude (inv_218 i 1)) s" by auto
from a4  have tmp:"formEval (inv_214 0)  s"   
            by (force simp del:inv_214_def) 
        have tmp1:"formEval (inv_214 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c5:"formEval  (conclude (inv_214 i)) s" by auto
from a4  have tmp:"formEval (inv_213 0 1)  s"   
            by (force simp del:inv_213_def) 
        have tmp1:"formEval (inv_213 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c6:"formEval  (conclude (inv_213 i 0)) s" by auto
from a4  have tmp:"formEval (inv_213 0 1)  s"   
            by (force simp del:inv_213_def) 
        have tmp1:"formEval (inv_213 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c7:"formEval  (conclude (inv_213 i 1)) s" by auto
from a4  have tmp:"formEval (inv_194 0)  s"   
            by (force simp del:inv_194_def) 
        have tmp1:"formEval (inv_194 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c8:"formEval  (conclude (inv_194 i)) s" by auto
from a4  have tmp:"formEval (inv_189 0 1)  s"   
            by (force simp del:inv_189_def) 
        have tmp1:"formEval (inv_189 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c9:"formEval  (conclude (inv_189 i 0)) s" by auto
from a4  have tmp:"formEval (inv_189 0 1)  s"   
            by (force simp del:inv_189_def) 
        have tmp1:"formEval (inv_189 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c10:"formEval  (conclude (inv_189 i 1)) s" by auto
from a4  have tmp:"formEval (inv_188 0 1)  s"   
            by (force simp del:inv_188_def) 
        have tmp1:"formEval (inv_188 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c11:"formEval  (conclude (inv_188 i 0)) s" by auto
from a4  have tmp:"formEval (inv_188 0 1)  s"   
            by (force simp del:inv_188_def) 
        have tmp1:"formEval (inv_188 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c12:"formEval  (conclude (inv_188 i 1)) s" by auto
from a4  have tmp:"formEval (inv_186 0 1)  s"   
            by (force simp del:inv_186_def) 
        have tmp1:"formEval (inv_186 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c13:"formEval  (conclude (inv_186 i 0)) s" by auto
from a4  have tmp:"formEval (inv_186 0 1)  s"   
            by (force simp del:inv_186_def) 
        have tmp1:"formEval (inv_186 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c14:"formEval  (conclude (inv_186 i 1)) s" by auto
from a4  have tmp:"formEval (inv_181 0)  s"   
            by (force simp del:inv_181_def) 
        have tmp1:"formEval (inv_181 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c15:"formEval  (conclude (inv_181 i)) s" by auto
from a4  have tmp:"formEval (inv_176 0)  s"   
            by (force simp del:inv_176_def) 
        have tmp1:"formEval (inv_176 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c16:"formEval  (conclude (inv_176 i)) s" by auto
from a4  have tmp:"formEval (inv_166 0)  s"   
            by (force simp del:inv_166_def) 
        have tmp1:"formEval (inv_166 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c17:"formEval  (conclude (inv_166 i)) s" by auto
from a4  have tmp:"formEval (inv_165 0 1)  s"   
            by (force simp del:inv_165_def) 
        have tmp1:"formEval (inv_165 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c18:"formEval  (conclude (inv_165 i 0)) s" by auto
from a4  have tmp:"formEval (inv_165 0 1)  s"   
            by (force simp del:inv_165_def) 
        have tmp1:"formEval (inv_165 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c19:"formEval  (conclude (inv_165 i 1)) s" by auto
from a4  have tmp:"formEval (inv_161 0)  s"   
            by (force simp del:inv_161_def) 
        have tmp1:"formEval (inv_161 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c20:"formEval  (conclude (inv_161 i)) s" by auto
from a4  have tmp:"formEval (inv_154 0)  s"   
            by (force simp del:inv_154_def) 
        have tmp1:"formEval (inv_154 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c21:"formEval  (conclude (inv_154 i)) s" by auto
from a4  have tmp:"formEval (inv_153 0 1)  s"   
            by (force simp del:inv_153_def) 
        have tmp1:"formEval (inv_153 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c22:"formEval  (conclude (inv_153 i 0)) s" by auto
from a4  have tmp:"formEval (inv_153 0 1)  s"   
            by (force simp del:inv_153_def) 
        have tmp1:"formEval (inv_153 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c23:"formEval  (conclude (inv_153 i 1)) s" by auto
from a4  have tmp:"formEval (inv_143 0)  s"   
            by (force simp del:inv_143_def) 
        have tmp1:"formEval (inv_143 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c24:"formEval  (conclude (inv_143 i)) s" by auto
from a4  have tmp:"formEval (inv_125 0 1)  s"   
            by (force simp del:inv_125_def) 
        have tmp1:"formEval (inv_125 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c25:"formEval  (conclude (inv_125 i 0)) s" by auto
from a4  have tmp:"formEval (inv_125 0 1)  s"   
            by (force simp del:inv_125_def) 
        have tmp1:"formEval (inv_125 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c26:"formEval  (conclude (inv_125 i 1)) s" by auto
from a4  have tmp:"formEval (inv_113 0)  s"   
            by (force simp del:inv_113_def) 
        have tmp1:"formEval (inv_113 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c27:"formEval  (conclude (inv_113 i)) s" by auto
from a4  have tmp:"formEval (inv_111 0 1)  s"   
            by (force simp del:inv_111_def) 
        have tmp1:"formEval (inv_111 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c28:"formEval  (conclude (inv_111 i 0)) s" by auto
from a4  have tmp:"formEval (inv_111 0 1)  s"   
            by (force simp del:inv_111_def) 
        have tmp1:"formEval (inv_111 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c29:"formEval  (conclude (inv_111 i 1)) s" by auto
from a4  have tmp:"formEval (inv_110 0 1)  s"   
            by (force simp del:inv_110_def) 
        have tmp1:"formEval (inv_110 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c30:"formEval  (conclude (inv_110 i 0)) s" by auto
from a4  have tmp:"formEval (inv_110 0 1)  s"   
            by (force simp del:inv_110_def) 
        have tmp1:"formEval (inv_110 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c31:"formEval  (conclude (inv_110 i 1)) s" by auto
from a4  have tmp:"formEval (inv_108 0)  s"   
            by (force simp del:inv_108_def) 
        have tmp1:"formEval (inv_108 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c32:"formEval  (conclude (inv_108 i)) s" by auto
from a4  have tmp:"formEval (inv_106 0)  s"   
            by (force simp del:inv_106_def) 
        have tmp1:"formEval (inv_106 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c33:"formEval  (conclude (inv_106 i)) s" by auto
from a4  have tmp:"formEval (inv_100 0)  s"   
            by (force simp del:inv_100_def) 
        have tmp1:"formEval (inv_100 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c34:"formEval  (conclude (inv_100 i)) s" by auto
from a4  have tmp:"formEval (inv_66 0 1)  s"   
            by (force simp del:inv_66_def) 
        have tmp1:"formEval (inv_66 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c35:"formEval  (conclude (inv_66 i 0)) s" by auto
from a4  have tmp:"formEval (inv_66 0 1)  s"   
            by (force simp del:inv_66_def) 
        have tmp1:"formEval (inv_66 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c36:"formEval  (conclude (inv_66 i 1)) s" by auto
from a4  have tmp:"formEval (inv_59 0 1)  s"   
            by (force simp del:inv_59_def) 
        have tmp1:"formEval (inv_59 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c37:"formEval  (conclude (inv_59 i 0)) s" by auto
from a4  have tmp:"formEval (inv_59 0 1)  s"   
            by (force simp del:inv_59_def) 
        have tmp1:"formEval (inv_59 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c38:"formEval  (conclude (inv_59 i 1)) s" by auto
from a4  have tmp:"formEval (inv_47 0 1)  s"   
            by (force simp del:inv_47_def) 
        have tmp1:"formEval (inv_47 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c39:"formEval  (conclude (inv_47 i 0)) s" by auto
from a4  have tmp:"formEval (inv_47 0 1)  s"   
            by (force simp del:inv_47_def) 
        have tmp1:"formEval (inv_47 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c40:"formEval  (conclude (inv_47 i 1)) s" by auto
from a4  have tmp:"formEval (inv_32 0 1)  s"   
            by (force simp del:inv_32_def) 
        have tmp1:"formEval (inv_32 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c41:"formEval  (conclude (inv_32 i 0)) s" by auto
from a4  have tmp:"formEval (inv_32 0 1)  s"   
            by (force simp del:inv_32_def) 
        have tmp1:"formEval (inv_32 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c42:"formEval  (conclude (inv_32 i 1)) s" by auto
from a4  have tmp:"formEval (inv_15 0 1)  s"   
            by (force simp del:inv_15_def) 
        have tmp1:"formEval (inv_15 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c43:"formEval  (conclude (inv_15 i 0)) s" by auto
from a4  have tmp:"formEval (inv_15 0 1)  s"   
            by (force simp del:inv_15_def) 
        have tmp1:"formEval (inv_15 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c44:"formEval  (conclude (inv_15 i 1)) s" by auto
from a4  have tmp:"formEval (inv_2 0 1)  s"   
            by (force simp del:inv_2_def) 
        have tmp1:"formEval (inv_2 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c45:"formEval  (conclude (inv_2 i 0)) s" by auto
from a4  have tmp:"formEval (inv_2 0 1)  s"   
            by (force simp del:inv_2_def) 
        have tmp1:"formEval (inv_2 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c46:"formEval  (conclude (inv_2 i 1)) s" by auto

      from b0 c0 c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 c26 c27 c28 c29 c30 c31 c32 c33 c34 c35 c36 c37 c38 c39 c40 c41 c42 c43 c44 c45 c46 show "formEval (pre ?r') s" 
       by auto
     qed
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
...
lemma lemmaOnn_RecvInvAck5LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_RecvInvAck5  i) (n_RecvInvAck5  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_RecvInvAck5: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_RecvInvAck5  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_RecvInvAck6Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_RecvInvAck6  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_RecvInvAck6  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_RecvInvAck6  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_RecvInvAck6  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_RecvInvAck6LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_RecvInvAck6  i) (n_RecvInvAck6  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_RecvInvAck6: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_RecvInvAck6  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendInvAck7Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendInvAck7  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendInvAck7  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendInvAck7  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendInvAck7  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendInvAck7LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendInvAck7  i) (n_SendInvAck7  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendInvAck7: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendInvAck7  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendInvAck8Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendInvAck8  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendInvAck8  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendInvAck8  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendInvAck8  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendInvAck8LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendInvAck8  i) (n_SendInvAck8  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendInvAck8: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendInvAck8  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendInv9Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendInv9  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendInv9  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendInv9  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendInv9  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendInv9LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendInv9  i) (n_SendInv9  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendInv9: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendInv9  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendInv10Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendInv10  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendInv10  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendInv10  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendInv10  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendInv10LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendInv10  i) (n_SendInv10  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendInv10: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendInv10  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_RecvReqE11Gt_i:
  ...
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC<N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
  shows "trans_sim_on2 (n_RecvReqE11 N i)  (n_RecvReqE11_i_3 ) VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF (set ?F) s")
  proof(rule ruleSimCond2)
    show " formEval (pre ?r) s ⟶formEval (pre ?r') s" (is "?A ⟶?B")
    proof(rule impI)+
      assume b0:"?A"
  from a4  have tmp:"formEval (inv_192 0)  s"   
            by (force simp del:inv_192_def) 
        have tmp1:"formEval (inv_192 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c0:"formEval  (conclude (inv_192 i)) s" by auto
from a4  have tmp:"formEval (inv_172 0)  s"   
            by (force simp del:inv_172_def) 
        have tmp1:"formEval (inv_172 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c1:"formEval  (conclude (inv_172 i)) s" by auto
from a4  have tmp:"formEval (inv_134 0)  s"   
            by (force simp del:inv_134_def) 
        have tmp1:"formEval (inv_134 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c2:"formEval  (conclude (inv_134 i)) s" by auto

      from b0 c0 c1 c2 show "formEval (pre ?r') s" 
       by auto
     qed
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
...
lemma lemmaOnn_RecvReqE11LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_RecvReqE11 N i) (n_RecvReqE11 N i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_RecvReqE11: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_RecvReqE11  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
  from a5 obtain i where d0:"i\<le>N\<and>r=n_RecvReqE11  i"  by blast
  have "i>NC|i\<le> NC" by auto
  moreover{
       assume a1:"i>NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
...
moreover{
       assume a1:"i\<le> NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
        proof(rule_tac x="(n_RecvReqE11 N i)" in exI,rule conjI)
          show  "?P1 (n_RecvReqE11 N i) " 
           by(cut_tac a1, auto) 
          next
          show  "?P2 (n_RecvReqE11 N i) "
...
lemma lemmaOnn_RecvReqS12Gt_i:
  ...
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC<N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
  shows "trans_sim_on2 (n_RecvReqS12 N i)  (n_RecvReqS12_i_3 ) VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF (set ?F) s")
  proof(rule ruleSimCond2)
    show " formEval (pre ?r) s ⟶formEval (pre ?r') s" (is "?A ⟶?B")
    proof(rule impI)+
      assume b0:"?A"
...
lemma lemmaOnn_RecvReqS12LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_RecvReqS12 N i) (n_RecvReqS12 N i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_RecvReqS12: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_RecvReqS12  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
  from a5 obtain i where d0:"i\<le>N\<and>r=n_RecvReqS12  i"  by blast
  have "i>NC|i\<le> NC" by auto
  moreover{
       assume a1:"i>NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
...
moreover{
       assume a1:"i\<le> NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
        proof(rule_tac x="(n_RecvReqS12 N i)" in exI,rule conjI)
          show  "?P1 (n_RecvReqS12 N i) " 
           by(cut_tac a1, auto) 
          next
          show  "?P2 (n_RecvReqS12 N i) "
...
lemma lemmaOnn_SendReqE13Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendReqE13  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendReqE13  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendReqE13  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendReqE13  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendReqE13LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendReqE13  i) (n_SendReqE13  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendReqE13: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendReqE13  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendReqE14Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendReqE14  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendReqE14  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendReqE14  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendReqE14  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendReqE14LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendReqE14  i) (n_SendReqE14  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendReqE14: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendReqE14  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_SendReqS15Gt_i:
  assumes a1:"NC<i" and a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
shows "trans_sim_on2 (n_SendReqS15  i  ) skipRule VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(unfold trans_sim_on2_def,(rule allI)+,(rule impI)+,rule disjI2)
  fix s2 
  assume b0:"state_sim_on2 s s2 VF "
  show "state_sim_on2 (trans (act (n_SendReqS15  i)) s) s2  VF"
  proof(cut_tac a1,unfold state_sim_on2_def,
    (rule allI)+,(rule impI)+)
    fix f v
    assume b1:" v ∈  VF"  
    
    have b5:"trans (act (n_SendReqS15  i)) s v = s v" 
      by (cut_tac a1  b1,auto) 

    have b6:"s v =s2 v "
      using b0 b1 state_sim_on2_def by blast  
    show "trans (act (n_SendReqS15  i)) s v= s2 v"
      using b5 b6 by auto 
  qed
qed
lemma lemmaOnn_SendReqS15LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendReqS15  i) (n_SendReqS15  i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendReqS15: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendReqS15  i"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
lemma lemmaOnn_StoreGt_i:
  assumes a1:"i>NC" and 
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC<N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"d\<le> NC"
  shows "trans_sim_on2 (n_Store  i d)  (n_Store_i_3 d) VF (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF (set ?F) s")
  proof(rule ruleSimCond2)
    show " formEval (pre ?r) s ⟶formEval (pre ?r') s" (is "?A ⟶?B")
    proof(rule impI)+
      assume b0:"?A"
  from a4  have tmp:"formEval (inv_236 0)  s"   
            by (force simp del:inv_236_def) 
        have tmp1:"formEval (inv_236 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c0:"formEval  (conclude (inv_236 i d)) s" by auto
from a4  have tmp:"formEval (inv_234 0)  s"   
            by (force simp del:inv_234_def) 
        have tmp1:"formEval (inv_234 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c1:"formEval  (conclude (inv_234 i d)) s" by auto
from a4  have tmp:"formEval (inv_231 0 1)  s"   
            by (force simp del:inv_231_def) 
        have tmp1:"formEval (inv_231 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c2:"formEval  (conclude (inv_231 i 0)) s" by auto
from a4  have tmp:"formEval (inv_231 0 1)  s"   
            by (force simp del:inv_231_def) 
        have tmp1:"formEval (inv_231 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c3:"formEval  (conclude (inv_231 i 1)) s" by auto
from a4  have tmp:"formEval (inv_226 0 1)  s"   
            by (force simp del:inv_226_def) 
        have tmp1:"formEval (inv_226 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c4:"formEval  (conclude (inv_226 i 0)) s" by auto
from a4  have tmp:"formEval (inv_226 0 1)  s"   
            by (force simp del:inv_226_def) 
        have tmp1:"formEval (inv_226 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c5:"formEval  (conclude (inv_226 i 1)) s" by auto
from a4  have tmp:"formEval (inv_224 0 1)  s"   
            by (force simp del:inv_224_def) 
        have tmp1:"formEval (inv_224 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c6:"formEval  (conclude (inv_224 i 0)) s" by auto
from a4  have tmp:"formEval (inv_224 0 1)  s"   
            by (force simp del:inv_224_def) 
        have tmp1:"formEval (inv_224 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c7:"formEval  (conclude (inv_224 i 1)) s" by auto
from a4  have tmp:"formEval (inv_217 0 1)  s"   
            by (force simp del:inv_217_def) 
        have tmp1:"formEval (inv_217 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c8:"formEval  (conclude (inv_217 i 0)) s" by auto
from a4  have tmp:"formEval (inv_217 0 1)  s"   
            by (force simp del:inv_217_def) 
        have tmp1:"formEval (inv_217 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c9:"formEval  (conclude (inv_217 i 1)) s" by auto
from a4  have tmp:"formEval (inv_209 0 1)  s"   
            by (force simp del:inv_209_def) 
        have tmp1:"formEval (inv_209 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c10:"formEval  (conclude (inv_209 i 0)) s" by auto
from a4  have tmp:"formEval (inv_209 0 1)  s"   
            by (force simp del:inv_209_def) 
        have tmp1:"formEval (inv_209 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c11:"formEval  (conclude (inv_209 i 1)) s" by auto
from a4  have tmp:"formEval (inv_194 0)  s"   
            by (force simp del:inv_194_def) 
        have tmp1:"formEval (inv_194 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c12:"formEval  (conclude (inv_194 i d)) s" by auto
from a4  have tmp:"formEval (inv_191 0)  s"   
            by (force simp del:inv_191_def) 
        have tmp1:"formEval (inv_191 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c13:"formEval  (conclude (inv_191 i d)) s" by auto
from a4  have tmp:"formEval (inv_190 0 1)  s"   
            by (force simp del:inv_190_def) 
        have tmp1:"formEval (inv_190 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c14:"formEval  (conclude (inv_190 i 0)) s" by auto
from a4  have tmp:"formEval (inv_190 0 1)  s"   
            by (force simp del:inv_190_def) 
        have tmp1:"formEval (inv_190 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c15:"formEval  (conclude (inv_190 i 1)) s" by auto
from a4  have tmp:"formEval (inv_180 0 1)  s"   
            by (force simp del:inv_180_def) 
        have tmp1:"formEval (inv_180 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c16:"formEval  (conclude (inv_180 i 0)) s" by auto
from a4  have tmp:"formEval (inv_180 0 1)  s"   
            by (force simp del:inv_180_def) 
        have tmp1:"formEval (inv_180 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c17:"formEval  (conclude (inv_180 i 1)) s" by auto
from a4  have tmp:"formEval (inv_164 0 1)  s"   
            by (force simp del:inv_164_def) 
        have tmp1:"formEval (inv_164 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c18:"formEval  (conclude (inv_164 i 0)) s" by auto
from a4  have tmp:"formEval (inv_164 0 1)  s"   
            by (force simp del:inv_164_def) 
        have tmp1:"formEval (inv_164 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c19:"formEval  (conclude (inv_164 i 1)) s" by auto
from a4  have tmp:"formEval (inv_159 0)  s"   
            by (force simp del:inv_159_def) 
        have tmp1:"formEval (inv_159 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c20:"formEval  (conclude (inv_159 i d)) s" by auto
from a4  have tmp:"formEval (inv_154 0)  s"   
            by (force simp del:inv_154_def) 
        have tmp1:"formEval (inv_154 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c21:"formEval  (conclude (inv_154 i d)) s" by auto
from a4  have tmp:"formEval (inv_138 0 1)  s"   
            by (force simp del:inv_138_def) 
        have tmp1:"formEval (inv_138 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c22:"formEval  (conclude (inv_138 i 0)) s" by auto
from a4  have tmp:"formEval (inv_138 0 1)  s"   
            by (force simp del:inv_138_def) 
        have tmp1:"formEval (inv_138 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c23:"formEval  (conclude (inv_138 i 1)) s" by auto
from a4  have tmp:"formEval (inv_136 0 1)  s"   
            by (force simp del:inv_136_def) 
        have tmp1:"formEval (inv_136 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c24:"formEval  (conclude (inv_136 i 0)) s" by auto
from a4  have tmp:"formEval (inv_136 0 1)  s"   
            by (force simp del:inv_136_def) 
        have tmp1:"formEval (inv_136 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c25:"formEval  (conclude (inv_136 i 1)) s" by auto
from a4  have tmp:"formEval (inv_106 0)  s"   
            by (force simp del:inv_106_def) 
        have tmp1:"formEval (inv_106 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c26:"formEval  (conclude (inv_106 i d)) s" by auto
from a4  have tmp:"formEval (inv_99 0)  s"   
            by (force simp del:inv_99_def) 
        have tmp1:"formEval (inv_99 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c27:"formEval  (conclude (inv_99 i d)) s" by auto
from a4  have tmp:"formEval (inv_89 0)  s"   
            by (force simp del:inv_89_def) 
        have tmp1:"formEval (inv_89 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c28:"formEval  (conclude (inv_89 i d)) s" by auto
from a4  have tmp:"formEval (inv_44 0 1)  s"   
            by (force simp del:inv_44_def) 
        have tmp1:"formEval (inv_44 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c29:"formEval  (conclude (inv_44 i 0)) s" by auto
from a4  have tmp:"formEval (inv_44 0 1)  s"   
            by (force simp del:inv_44_def) 
        have tmp1:"formEval (inv_44 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c30:"formEval  (conclude (inv_44 i 1)) s" by auto
from a4  have tmp:"formEval (inv_17 0)  s"   
            by (force simp del:inv_17_def) 
        have tmp1:"formEval (inv_17 i d  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c31:"formEval  (conclude (inv_17 i d)) s" by auto

      from b0 c0 c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 c26 c27 c28 c29 c30 c31 show "formEval (pre ?r') s" 
       by auto
     qed
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
...
lemma lemmaOnn_StoreLeNc_:
  assumes a1:"i\<le>NC & d\<le>NC" 
  shows "trans_sim_on2 (n_Store  i d) (n_Store  i d) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_Store: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i d. i\<le>N\<and>d\<le>N\<and>r=n_Store  i d"  and
  a6:"NC<N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
  from a5 obtain i d where d0:"i\<le>N\<and>d\<le>N\<and>r=n_Store  i d"  by blast
  have "i>NC&d\<le> NC|i\<le> NC & d\<le> NC" by auto
  moreover{
       assume a1:"i>NC&d\<le> NC" 
        have "∃r'. ?P1 r' ∧ ?P2 r'"
