...
axiomatization  where axiomOnf2 [simp,intro]:
   "s ∈ reachableSet (set (allInitSpecs N )) (rules N) ⟹  1 \<le> N ⟹i\<noteq>j ⟹ 
 j \<le> N ⟹ formEval (f 0 1) s ⟹ formEval (f i j) s"

axiomatization  where axiomOnf1 [simp,intro]:
   "s ∈ reachableSet (set (allInitSpecs N )) (rules N)  ⟹formEval (f 0 ) s ⟹ formEval (f i) s"
...
lemma lemmaOnn_SendGntE3Gt_i:
  assumes a1:"i>NC" and 
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC\<le>N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" 
...
from a4  have tmp:"formEval (inv_91)  s"   
      ...
      from a3 b0 c0 c1 c2 c3 c4 show "formEval (pre ?r') s" 
...
proof(rule ruleSimCond2)
     from a3 show " formEval (pre ?r) s ⟶formEval (pre ?r') s" (is "?A ⟶?B")  
       apply (auto simp del:evalEqn)done
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
    expEval (substExpByStatement (IVar v)  (act ?r')) s = expEval (substExpByStatement (IVar v)  (act ?r)) s)"
...
lemma lemmaOnn_SendGntE3LeNc_1:
  assumes a1:"i\<le>NC"  and a3:"NC\<le>N"  
  shows "trans_sim_on2 (n_SendGntE3 N i) (n_SendGntE3 NC i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r' VF ?F s")
proof(rule ruleSimCond2)
...
lemma lemmaOnn_SendGntE3LeNc_:
  assumes a1:"i\<le>NC" 
  shows "trans_sim_on2 (n_SendGntE3 N i) (n_SendGntE3 N i) VF  (set invariantsAbs) s" (is "trans_sim_on2 ?r ?r VF ?F s")
proof(rule ruleSimId2)
  show  "∀v. v∈varOfForm (pre ?r) ⟶  v ∈ VF "
    by(cut_tac a1, auto) 
...
lemma lemmaOnn_SendGntE3: 
  assumes   a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"\<exists> i. i\<le>N\<and>r=n_SendGntE3 N i"  and
  a6:"NC\<le>N"
  shows "∃ r'. r' ∈ rules NC∧ trans_sim_on2 r r' VF (set invariantsAbs) s" (is "∃r'.?P1 r' ∧ ?P2 r'")
proof -
...
from a4  have tmp:"formEval (inv_91)  s"   
      ...
      from a3 b0 c0 c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 show "formEval (pre ?r') s" 
       by auto
     qed
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
...
lemma lemmaOnn_RecvInvAck5Gt_i:
  assumes a1:"i>NC" and 
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC\<le>N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a5:"i \<le> N"
  ...
  from a4  have tmp:"formEval (inv_233 0)  s"   
        ...
        have tmp1:"formEval (inv_233 i  ) s" thm axiomOnf1
        proof(cut_tac a1 a2 a3 a5 tmp,rule axiomOnf1,force+)qed
...
from a4  have tmp:"formEval (inv_106 0)  s"   
            by (force simp del:inv_106_def)
        	 with b0 a1 have c33:"formEval  (conclude (inv_106 0)) s" by auto
from a4  have tmp:"formEval (inv_106 1)  s"   
            by (force simp del:inv_106_def)
        	 with b0 a1 have c34:"formEval  (conclude (inv_106 1)) s" by auto
from a4  have tmp:"formEval (inv_100 0)  s"   
            by (force simp del:inv_100_def) 
        have tmp1:"formEval (inv_100 i  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf1,force+)qed
        with b0 a1 have c35:"formEval  (conclude (inv_100 i)) s" by auto
from a4  have tmp:"formEval (inv_66 0 1)  s"   
            by (force simp del:inv_66_def) 
        have tmp1:"formEval (inv_66 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c36:"formEval  (conclude (inv_66 i 0)) s" by auto
from a4  have tmp:"formEval (inv_66 0 1)  s"   
            by (force simp del:inv_66_def) 
        have tmp1:"formEval (inv_66 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c37:"formEval  (conclude (inv_66 i 1)) s" by auto
from a4  have tmp:"formEval (inv_59 0 1)  s"   
            by (force simp del:inv_59_def) 
        have tmp1:"formEval (inv_59 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c38:"formEval  (conclude (inv_59 i 0)) s" by auto
from a4  have tmp:"formEval (inv_59 0 1)  s"   
            by (force simp del:inv_59_def) 
        have tmp1:"formEval (inv_59 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c39:"formEval  (conclude (inv_59 i 1)) s" by auto
from a4  have tmp:"formEval (inv_47 0 1)  s"   
            by (force simp del:inv_47_def) 
        have tmp1:"formEval (inv_47 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c40:"formEval  (conclude (inv_47 i 0)) s" by auto
from a4  have tmp:"formEval (inv_47 0 1)  s"   
            by (force simp del:inv_47_def) 
        have tmp1:"formEval (inv_47 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c41:"formEval  (conclude (inv_47 i 1)) s" by auto
from a4  have tmp:"formEval (inv_32 0 1)  s"   
            by (force simp del:inv_32_def) 
        have tmp1:"formEval (inv_32 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c42:"formEval  (conclude (inv_32 i 0)) s" by auto
from a4  have tmp:"formEval (inv_32 0 1)  s"   
            by (force simp del:inv_32_def) 
        have tmp1:"formEval (inv_32 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c43:"formEval  (conclude (inv_32 i 1)) s" by auto
from a4  have tmp:"formEval (inv_15 0 1)  s"   
            by (force simp del:inv_15_def) 
        have tmp1:"formEval (inv_15 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c44:"formEval  (conclude (inv_15 i 0)) s" by auto
from a4  have tmp:"formEval (inv_15 0 1)  s"   
            by (force simp del:inv_15_def) 
        have tmp1:"formEval (inv_15 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c45:"formEval  (conclude (inv_15 i 1)) s" by auto
from a4  have tmp:"formEval (inv_2 0 1)  s"   
            by (force simp del:inv_2_def) 
        have tmp1:"formEval (inv_2 i 0  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c46:"formEval  (conclude (inv_2 i 0)) s" by auto
from a4  have tmp:"formEval (inv_2 0 1)  s"   
            by (force simp del:inv_2_def) 
        have tmp1:"formEval (inv_2 i 1  ) s" 
        proof(cut_tac a1 a2 a3 tmp,rule axiomOnf2,force+)qed
        with b0 a1 have c47:"formEval  (conclude (inv_2 i 1)) s" by auto

      from a3 b0 c0 c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 c13 c14 c15 c16 c17 c18 c19 c20 c21 c22 c23 c24 c25 c26 c27 c28 c29 c30 c31 c32 c33 c34 c35 c36 c37 c38 c39 c40 c41 c42 c43 c44 c45 c46 c47 show "formEval (pre ?r') s" 
       by auto
     qed
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
   ...
   proof(rule allI,(rule impI)+)
     ...
     from a4  have tmp:"formEval (inv_100 0)  s"   
        ...
        with b4 a1 have c47:"formEval  (conclude (inv_100 i )) s" by auto
  show "expEval (substExpByStatement (IVar v)  (act ?r')) s = expEval (substExpByStatement (IVar v)  (act ?r)) s"  
    apply (cut_tac  a1 a3 a5 b1 b2 b3 b4 c47,auto)
    done
   qed
 next
   show "∀  v. v ∈  varOfSent (act ?r) ⟶  v ∈ VF ⟶ v ∈  varOfSent (act ?r')" by(cut_tac a1, auto)
...
lemma lemmaOnn_RecvReqE11Gt_i:
  ...
  from a4  have tmp:"formEval (inv_192 0)  s"   
            by (force simp del:inv_192_def)
        	 with b0 a1 have c0:"formEval  (conclude (inv_192 0)) s" by auto
from a4  have tmp:"formEval (inv_192 1)  s"   
            by (force simp del:inv_192_def)
        	 with b0 a1 have c1:"formEval  (conclude (inv_192 1)) s" by auto
from a4  have tmp:"formEval (inv_172 0)  s"   
            by (force simp del:inv_172_def)
        	 with b0 a1 have c2:"formEval  (conclude (inv_172 0)) s" by auto
from a4  have tmp:"formEval (inv_172 1)  s"   
            by (force simp del:inv_172_def)
        	 with b0 a1 have c3:"formEval  (conclude (inv_172 1)) s" by auto
from a4  have tmp:"formEval (inv_134 0)  s"   
            by (force simp del:inv_134_def)
        	 with b0 a1 have c4:"formEval  (conclude (inv_134 0)) s" by auto
from a4  have tmp:"formEval (inv_134 1)  s"   
            by (force simp del:inv_134_def)
        	 with b0 a1 have c5:"formEval  (conclude (inv_134 1)) s" by auto

      from a3 b0 c0 c1 c2 c3 c4 c5 show "formEval (pre ?r') s" 
       by auto
     qed
   next 
	show "(∀  v. v ∈  varOfSent (act  ?r') ⟶  v ∈ VF ⟶ formEval (pre ?r) s ⟶ 
  ...
  show "expEval (substExpByStatement (IVar v)  (act ?r')) s = expEval (substExpByStatement (IVar v)  (act ?r)) s"  
    apply (cut_tac  a1 b1 b2 b3 a5,auto)
    done
   qed
 next
   show "∀  v. v ∈  varOfSent (act ?r) ⟶  v ∈ VF ⟶ v ∈  varOfSent (act ?r')" by(cut_tac a1, auto)
...
next
  show  b1: "∀v a. a ∈ set (statement2Assigns (act ?r)) ⟶ 
v∈varOfExp ( substExpByStatement (IVar (fst a))  (act ?r))⟶v ∈VF "
   proof(cut_tac a1,(rule allI)+,(rule impI)+,auto) qed
...
lemma lemmaOnn_RecvReqS12Gt_i:
  assumes a1:"i>NC" and 
  a2:"s ∈ reachableSet (set (allInitSpecs N)) (rules N)"  and a3:"NC\<le>N" and  
  a4:"∀f.  f ∈(set invariantsAbs) ⟶  formEval f s" and a6:"i \<le>N"
...
from a4  have tmp:"formEval (inv_134 1)  s"   
  ...
  show "expEval (substExpByStatement (IVar v)  (act ?r')) s = expEval (substExpByStatement (IVar v)  (act ?r)) s"  
    apply (cut_tac  a1 a3 b1 b2 b3 a6,auto)
    done
   qed
 next
   show "∀  v. v ∈  varOfSent (act ?r) ⟶  v ∈ VF ⟶ v ∈  varOfSent (act ?r')" by(cut_tac a1, auto)
